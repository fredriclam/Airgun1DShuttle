...
[solution_leak_rr0, metadata_leak_rr0] = airgunShuttleDeploy(nx, true, ...
    struct(...
        'bubbleModel', ...
            struct('type', 'single', 'waterProperties', struct(), 'flowreductionfactor', 1.0), ...
...
[solution_leak_rr1, metadata_leak_rr1] = airgunShuttleDeploy(nx, true, ...
    struct(...
        'bubbleModel', ...
            struct('type', 'single', 'waterProperties', struct(), 'flowreductionfactor', 0.75), ...
    'extraOptions', struct('TInitial', 288, 'pb', 1*198000), ...
    'leakTime', 0.0));
[solution_leak_rr2, metadata_leak_rr2] = airgunShuttleDeploy(nx, true, ...
    struct(...
        'bubbleModel', ...
            struct('type', 'single', 'waterProperties', struct(), 'flowreductionfactor', 0.5), ...
    'extraOptions', struct('TInitial', 288, 'pb', 1*198000), ...
    'leakTime', 0.0));
[solution_leak_rr3, metadata_leak_rr3] = airgunShuttleDeploy(nx, true, ...
    struct(...
        'bubbleModel', ...
            struct('type', 'single', 'waterProperties', struct(), 'flowreductionfactor', 0.25), ...
    'extraOptions', struct('TInitial', 288, 'pb', 1*198000), ...
    'leakTime', 0.0));
...
plot(fieldData.t, fieldData.y, 'k.');
hold on
% Plot models
lw = 1.5;
plot(1e3*solution_leak_rr0.soln.x, 1e-6*p_L_fn(solution_leak_rr0.q(1:3,:)), '-', "LineWidth", lw);
plot(1e3*solution_leak_rr1.soln.x, 1e-6*p_L_fn(solution_leak_rr1.q(1:3,:)), '-', "LineWidth", lw);
plot(1e3*solution_leak_rr2.soln.x, 1e-6*p_L_fn(solution_leak_rr2.q(1:3,:)), '-', "LineWidth", lw);
plot(1e3*solution_leak_rr3.soln.x, 1e-6*p_L_fn(solution_leak_rr3.q(1:3,:)), '-', "LineWidth", lw);

xlim([0,1e3*0.3])
ylim([0, 8])
xlabel("t (ms)")
ylabel("pL (MPa)")
legend(["Data", "\eta = 1.0", "\eta = 0.75", "\eta = 0.5", "\eta = 0.25"])
