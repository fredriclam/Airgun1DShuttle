...
%% Foreground run
[solution_leak, metadata_leak] = airgunShuttleDeploy(nx, true, ...
    struct('bubbleModel', ...
        struct('type', 'single', 'waterProperties', struct()), ...
    'extraOptions', struct('TInitial', 288), ...
    'leakTime', 1.0));
...
%% Sample signatures
[p_aco_wet, funcs_wet] = agtools.sampleSignature( ...
    solution_wet, metadata_wet);
%%
[p_aco_ref, funcs_ref] = agtools.sampleSignature( ...
    solution_reference, metadata_reference);
%%
[p_aco_leak, funcs_leak] = agtools.sampleSignature( ...
    solution_leak, metadata_leak);
...
%%
figure(5); clf;
t_long = linspace(0,10,1500);
plot(t_long, funcs_ref.pFn(t_long));
hold on
plot(t_long, funcs_wet.pFn(t_long));
hold off
