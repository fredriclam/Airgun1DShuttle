...
if metadata.usingShuttleModel
    ...
    caseKeySwitchIndices = [0, find(...
        ...
        length(caseKeyHistory)-1), length(caseKeyHistory)];
    ...
    for i = 1:length(caseKeySwitchIndices)-1
        ...
        plotRange = caseKeySwitchIndices(i)+1:...
            min(caseKeySwitchIndices(i+1)+1,length(caseKeyHistory));
    ...
    text(0.0, 0.7, sprintf('Initial $p$: $%.1f$ MPa = $%.0f$ psi', ...
        ...
        'FontSize', 13);
    text(0.0, 0.6, sprintf('Mid chamber: %s', ...
        metadata.paramAirgun.midChamberMode), 'Interpreter', 'latex', ...
        'FontSize', 13);
    
    
    text(0.0, 0.4, sprintf('Wall clock time: $%.1f$ s', ...
