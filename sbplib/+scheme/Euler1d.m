classdef Euler1d < scheme.Scheme
    properties
        ...
        % Manually instantiate dummy grid variable -- FL
        grid
...
%                 w_s = inv(T)*q_s;
        ...
        % Return closure(q, p)
        function closure = boundary_condition_outflow_vel(obj, boundary)
            s = obj.getBoundarySign(boundary);
        ...
        % Set wall boundary condition rho*v = something.
        function closure = boundary_condition_outflow_rhovel(obj, boundary)
            [e_s, e_S] = obj.getBoundaryOperator({'e', 'E'}, boundary);
            s = obj.getBoundarySign(boundary);
            ...
            function o = closure_fun(q, rhovel)
